...
@train.command("binary")
@click.option("-x", "--text-column", required=True, type=str)
@click.option("-y", "--label-column", required=True, type=str)
@click.pass_obj
def binary(ctx, text_column, label_column):
    texts, labels = load_classification_data(
...
@click.option("-y", "--label-column", required=True, type=str)
@click.pass_obj
def multiclass(ctx, text_column, label_column):
    texts, labels = load_classification_data(
        ctx["input_data"], text_column=text_column, label_column=label_column
    )
    training_dataloader = create_training_dataloader(
...
@click.option("-x", "--text-column", required=True, type=str)
@click.argument("label_columns", required=True, nargs=-1, type=str)
@click.pass_obj
def multilabel(ctx, text_column, label_columns):
    # TODO
    texts, labels = load_multilabel_data(
        ctx["input_data"], text_column=text_column, label_columns=list(label_columns)
    )
    training_dataloader = create_training_dataloader_multilabel(
        texts, labels, ctx["max_seq_len"], ctx["batch_size"]
    )
...
@click.option("-n", "--model-name", type=str, nargs=1)
@click.option("-b", "--batch-size", type=int, default=TRAIN_BATCH_SIZE, nargs=1)
def predict(input_data, export_path, model_name, batch_size):
    ...
    if problem_type in ("binary", "multiclass"):
        model = load_model_classification(
            model_path, model_name, num_labels=num_labels
        )
        prediction_loader = create_prediction_dataloader(
            texts, max_seq_len, batch_size
        )
